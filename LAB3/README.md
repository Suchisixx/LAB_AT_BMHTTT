...
| :---: | :--- | :--- | :--- | :---: |
| **TH1** | **Môi trường & Baseline** | Ghi nhận cấu hình hệ điều hành, tài nguyên phần cứng, phiên bản các công cụ bảo vệ và baseline ban đầu của Defender/Firewall. | `H1_VM_WindowsVersion.png`, `H2_ToolVersion.png`, `H3_Baseline_Defender_Firewall.png` | **PASS** |
| **TH2** | **Malware & AV Evasion (EICAR)** | Ghi nhận chuỗi kiểm thử EICAR, kích hoạt tính năng Real-time Protection và Scan của Windows Defender; phân tích phát hiện trong Protection History. | `H4_ProtectionHistory_EICAR.png` | **PASS** |
| **TH3** | **Password Attacks & Keylogging Analysis** | Bật chính sách Audit Logon, tạo tài khoản thử nghiệm `lab3user`, mô phỏng các lần đăng nhập thất bại (Event 4625); thực hiện chu trình đổi mật khẩu (Credential Rotation) và phân tích nguy cơ Keylogger. | `H5_Event4625.png`, `auth_events_before_rotation.txt` | **PASS** |
| **TH4** | **Backdoor, Persistence & Listener** | Cài đặt Sysmon theo dõi tiến trình (Event 1); cắm Persistence lành tính qua Registry Run Key (`LAB3_Run_Demo`); khởi chạy HTTP listener trên cổng `127.0.0.1:8080`, ánh xạ PID qua Process Explorer. | `H6_Sysmon_Event1.png`, `H7_Autoruns_LAB3_Run_Demo.png`, `H8_ProcessExplorer_Python.png` | **PASS** |
| **TH5** | **Sniffing & MITM: So sánh HTTP vs HTTPS** | Bắt gói tin Loopback phát hiện dữ liệu bản rõ (plaintext) qua HTTP 8080; chuyển NAT bắt gói cổng 443 chứng minh payload được mã hóa bảo vệ (Encrypted Application Data) qua TLS. | `H9_HTTP_Plaintext.png`, `H10_TLS_443.png` | **PASS** |
| **TH6** | **DoS / DDoS / Mail Bombing** | Chạy kiểm thử tải nội bộ (`local_load_test.py`) tới `127.0.0.1:8080`, quan sát tăng tải hệ thống; phân tích log email (Mail bombing) làm tràn hộp thư. | `H10_Load_and_Log_Analysis.png`, `H11_Maillog.png` | **PASS** |
| **TH7** | **Social Engineering & Phishing Analysis** | Phân tích thông điệp email lừa đảo offline (Sender Spoofing, Urgency, Hyperlink giả mạo cổng đăng nhập nhằm đánh cắp tài khoản); phân tích yếu tố cá nhân hóa Spear Phishing. | `H12_Phishing_Offline.png` | **PASS** |
| **TH8** | **Cô lập, Cleanup, Phục hồi & Kiểm tra lại** | Dừng tiến trình HTTP listener cổng 8080, xóa bỏ khóa Registry Run `LAB3_Run_Demo`, hủy Scheduled Task, vô hiệu hóa tài khoản `lab3user`; kiểm tra toàn vẹn hệ thống và cập nhật mã băm SHA-256. | `evidence_sha256.csv` | **PASS** |
